...
size_t Marker::createIB(size_t ap_count)
{
    ...
    const std::vector<unsigned int> marker_idx{0, 1, 2, 0, 1, 4, 0, 4, 3, 0, 3, 2, 1, 4, 2, 4, 2, 3};

    for (unsigned n = 0; n < ap_count; n++)
    {
        unsigned int offset = n * VERTEX_BY_MARKER_COUNT_;

        std::for_each(marker_idx.begin(), marker_idx.end(), [&](unsigned int idx) { index.push_back(offset + idx); });
    }
