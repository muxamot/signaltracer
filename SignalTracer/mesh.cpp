...
namespace sgtr {
	...
	Mesh::Mesh(const vertex_buf_t& vertices, const index_buf_t& indexes)
	{
		...
		if (vbo_ == INVALID_OGL_VALUE || ibo_ == INVALID_OGL_VALUE) {
			...
			throw std::exception("Failed to build OpenGL buffers");
		}
	...
	size_t Mesh::getIndexCount() const { return index_count_; }
	GLuint Mesh::getIndexBuffer() const { return ibo_; }
	GLuint Mesh::getVertexBuffer() const { return vbo_; }
}