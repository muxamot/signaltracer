...
namespace sgtr {
	...
	void Shaders::read(const std::string& filename, std::string& stringbuf)
	{
		...
		if (!fs) {
			...
			throw std::exception("Error during file opening");
		}
	...
	void Shaders::add(GLenum type)
	{
		...
		if (ShaderObj == 0) {
			...
			throw std::exception("Error during shader adding");
		}
	...
	void Shaders::compile()
	{
		...
		if (!pshader_) {
			...
			throw std::exception("Shader program build error");
		}
		...
		if (!Success) {
			...
			LOG(ERROR) << ErrorLog;
			throw std::exception("OpenGL validation fail");
		}
	...
	GLuint Shaders::setUniform(const std::string & name)
	{
		...
		if (loc == 0xFFFFFFFF) {
			...
			throw std::exception("bad address given");
		}
